...
public class EventAdmin extends Basetest {
	...
	public void searchEventCreatedByAdminForBusinessUserAndEditThatEvent() throws InterruptedException {
		...
		Actions actions = new Actions(driver);
		WebElement cloneTheEvent = wait.until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//ul[@class='dropdown-menu show']//a[@class='dropdown-item'][normalize-space()='Edit']")));
		actions.moveToElement(cloneTheEvent).click().perform();
		Thread.sleep(1000);
		...
		eventDescription.sendKeys(
				"Get ready to embark on a culinary adventure like noet Dining Experience! Indulge TEST 123232");
	...
	public void searchEventCreatedByAdminForBusinessUserAndCancleThatEvent() throws InterruptedException {
		...
		reuse.reusebaleCodeForEventdashboard();
		Actions actions = new Actions(driver);
		WebElement cloneTheEvent = wait.until(ExpectedConditions
